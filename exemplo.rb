Vagrant.configure("2") do |config|
    ...
    config.vm.define "dhcp-server" do |dhcp|
      ...
      dhcp.vm.provision "shell", inline: <<-SHELL
        # Configuração do servidor DHCP usando Docker
    ...
    config.vm.define "dns-server" do |dns|
      ...
      dns.vm.provision "shell", inline: <<-SHELL
        # Configuração do servidor DNS usando Docker
    ...
    config.vm.define "web-server" do |web|
      ...
      web.vm.provision "shell", inline: <<-SHELL
        # Configuração do servidor web usando Docker (Exemplo com Nginx)
    ...
    config.vm.define "ftp-server" do |ftp|
      ...
      ftp.vm.provision "shell", inline: <<-SHELL
        # Configuração do servidor FTP usando Docker (Exemplo com vsftpd)
    ...
    config.vm.define "nfs-server" do |nfs|
      ...
      nfs.vm.provision "shell", inline: <<-SHELL
        # Configuração do servidor NFS usando Docker
